    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                VehicleDetailRow(label = "Price", value = "AED AED{"%.2f".format(vehicle.price)}")
                VehicleDetailRow(label = "Deposit", value = vehicle.deposit?.let { "AED AED{"%.2f".format(it)}" } ?: "—")
                VehicleDetailRow(label = "Installment", value = vehicle.installment?.let { "AED AED{"%.2f".format(it)}" } ?: "—")
                VehicleDetailRow(label = "Installment Duration", value = vehicle.installmentDurationMonths?.let { "AEDit months" } ?: "—")
                ...
                VehicleDetailRow(label = "Insurance", value = "AED AED{"%.2f".format(vehicle.annualInsuranceAmount)}")
                VehicleDetailRow(label = "Fuel Tank", value = vehicle.fuelTankCapacity?.let { "AEDit L" } ?: "—")
                VehicleDetailRow(label = "Fuel Consumption", value = vehicle.fuelConsumptionPer100Km?.let { "AEDit L/100km" } ?: "—")
